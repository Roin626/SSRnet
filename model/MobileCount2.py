...
class MobileCount(nn.Module):
    ...
    def __init__(self, num_classes=1, pretrained=False):
        ...
        #                           padding=1, bias=True)
        self.clf_conv = nn.Conv2d(32, 3, kernel_size=3, stride=1,
                                  padding=1, bias=True)
        # self.convDU = convDU(in_out_channels=32,kernel_size=(1,9))
        # self.convLR = convLR(in_out_channels=32,kernel_size=(9,1))
        self.level_class = levelclass(input_channel=32, pred_shapes=[1, 1, 32, 32])
    ...
    def forward(self, x):
        ...
        x1 = self.dropout_clf(x1)#8,32,192,144

        # x = self.convDU(x1)
        # x = self.convLR(x)
        count = self.level_class(x1)
...
class MobileCountSSR(nn.Module):

    def __init__(self, num_classes=1, pretrained=False):
        self.inplanes = 32
        block = Bottleneck
        layers = [1, 2, 3, 4]
        super(MobileCountSSR, self).__init__()
        ...
        self.up1 = nn.Sequential(
            SSRupsampling2(inplanes=256, outplanes=64, scale=16, k_size=1, pad=0),
            nn.ReLU(inplace=True),
        )
        self.up5 = UNetcat(64, 32)
        ...
        # self.clf_conv = nn.Conv2d(256, num_classes, kernel_size=3, stride=1,
        #                           padding=1, bias=True)
        self.clf_conv = nn.Conv2d(64, 3, kernel_size=3, stride=1,
                                  padding=1, bias=True)
        # self.convDU = convDU(in_out_channels=32,kernel_size=(1,9))
        # self.convLR = convLR(in_out_channels=32,kernel_size=(9,1))
